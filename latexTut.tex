...
\begin{itemize}
    ...
    \item Basic super- and sub-scripts can be written using \verb|^| and \verb|_|, respectively, when in an equation environment. For example, $F^2$ is written \verb|$F^2$|.
...
To change the literals displayed on the K-Map, change the optional arguments \texttt{A}, \texttt{B},\texttt{C}, and \texttt{D} to the desired values. Note that mathmode is automatically implied, so writing \verb|[S_1]| will work, while \verb|[$S_1$]| will not.
